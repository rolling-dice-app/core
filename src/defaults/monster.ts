...
/** 新建怪物時非 MonsterTemplateCreateBody 欄位的初始值；frontend mock 與 backend POST handler 共用。id / userId / 時間戳 由 backend row state 帶出，不屬 defaults */
export const buildMonsterTemplateCreateDefaults = (): Omit<
  MonsterTemplateDTO,
  keyof MonsterTemplateCreateBody | 'id' | 'userId' | 'createdAt' | 'updatedAt'
> => ({
  size: null,
  alignment: null,
